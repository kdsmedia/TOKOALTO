...
const commands = {
  ...
  "cek nomor": async (conn) => {
    let nomor = await pertanyaan("masukkan nomor wa : ");
    nomor = nomor.split(",");
    let i = 0;
    for (let el of nomor) {
    ...
    let simpan = await pertanyaan("y/n (default n) simpan? : ");
    if (simpan == "y") {
...
console.log(
  "HALLO SAHABAT!!!\nini adalah WHATSAPP PINTAR\nyang dikembangkan oleh ALTO MEDIA\nselamat mencoba\nselamat menikmati fitur-fiturnya"
);
//let i = 0;
//console.log("berikut ini daftar command yang tersedia");
//for (const c in commands) {
